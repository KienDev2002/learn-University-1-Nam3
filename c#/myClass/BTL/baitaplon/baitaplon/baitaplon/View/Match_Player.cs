...
namespace baitaplon
{
    ...
    public partial class Match_Player : Form
    {
        ...
        ProcessConnect conn = new ProcessConnect("Data Source=DESKTOP-OSRNAUL\\SQLEXPRESS;Initial Catalog=QLGiaiBongNHA;Integrated Security=True");
        public void reset()
        {
        ...
        private void Formdgvtrandau_cauthu_Load(object sender, EventArgs e)
        {
            ...
            getData("select distinct MaTD from TranDau_CauThu", cbShowClub, "MaTD");
            getData("select distinct TenDoi from TranDau_CauThu join TranDau on TranDau.MaTD = TranDau_CauThu.MaTD join DoiBong on DoiBong.MaDoi = TranDau.MaDoiKhach or DoiBong.MaDoi = TranDau.MaDoiNha", cbShowPlayer, "TenDoi");
