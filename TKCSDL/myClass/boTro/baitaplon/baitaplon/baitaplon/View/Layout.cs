...
namespace baitaplon
{
    public partial class Layout : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            btnClub.Size = new System.Drawing.Size(262, 70);
            btnPlayer.Size = new System.Drawing.Size(262, 70);
            btnMatch.Size = new System.Drawing.Size(262, 70);
            StateSearch(false);
        }
        ...
        private void picImageRemove_Click(object sender, EventArgs e)
        {
            ...
            else if (lbtitle.Text.Equals("Match"))
            {
                ...
                dt_the = db.getTable($"select * from TranDau_The where MaTD = N'{ma}'");


                DataTable dt_banThang = new DataTable();
                dt_banThang = db.getTable($"select * from TranDau_BanThang where MaTD = N'{ma}'");


                DataTable dt_cauThu = new DataTable();
                dt_cauThu = db.getTable($"select * from TranDau_CauThu where MaTD = N'{ma}'");
                ...
                if (MessageBox.Show("Bạn có muốn xóa trận đấu không?", "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
                {
                    try
                    {
                        ...
                        {
                            db.Excute($"delete from TranDau_The where MaTD = N'{ma}'");
                        }

                        if (dt_banThang.Rows.Count > 0)
                        {
                            db.Excute($"delete from TranDau_BanThang where MaTD = N'{ma}'");
                        }


                        if (dt_cauThu.Rows.Count > 0)
                        {
                            db.Excute($"delete from TranDau_CauThu where MaTD = N'{ma}'");
                        }
