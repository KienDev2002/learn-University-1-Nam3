...
tKhachHang.MaKH, TenKH, sum(SLBan * DonGiaBan) as TongTienTieuDung
...
begin
	...
	update tHoaDonBan set TongTien=isnull(TongTien,0)+isnull(@inTien,0) -isnull(@deTien,0) where SoHDB = isnull(@insohdb, @desohdb)
end
...
begin
	...
	select @inncc = MaNCC, @inTien = SLNhap * tSach.DonGiaNHap, @inSL=SLNhap from inserted join tSach on tSach.MaSach = inserted.MaSach join tHoaDonNhap on inserted.SoHDN=tHoaDonNhap.SoHDN
	select @dencc = MaNCC, @deTien = SLNhap * tSach.DonGiaNhap, @deSL=SLNhap from deleted join tSach on tSach.MaSach = deleted.MaSach join tHoaDonNhap on deleted.SoHDN=tHoaDonNhap.SoHDN
	update tNhaCungCap set TongTien=isnull(TongTien,0)+isnull(@inTien,0), SLSach=isnull(SLSach,0)+isnull(@inSL,0) where MaNCC = @inncc
	update tNhaCungCap set TongTien=isnull(TongTien,0)-isnull(@deTien,0), SLSach=isnull(SLSach,0)-isnull(@deSL,0) where MaNCC = @inncc
end
...
update tHoaDonBan set SoSP=isnull(SoSP,0)+isnull(@sl,0)-isnull(@sl1,0) where  sohdb=isnull(@sohdb , @sohdb1)
End
...
begin
	...
	from (
		select MaSach, sum(SLNhap) as TongSoLuong 
		from inserted 
		group by MaSach
	) as tmp
	...
	update tSach
	set SoLuong = isnull(SoLuong, 0) - isnull(TongSoLuong, 0)
	from (select MaSach, sum(SLNhap) as TongSoLuong from deleted group by MaSach) as tmp
	where tSach.MaSach = tmp.MaSach
end

create trigger BanSach
	on tChiTietHDB
	for insert, update, delete as
begin
	update tSach
	set SoLuong = isnull(SoLuong, 0) - isnull(TongSoLuong, 0)
	from (select MaSach, sum(SLBan) as TongSoLuong from inserted group by MaSach) as tmp
	where tSach.MaSach = tmp.MaSach

	update tSach
	set SoLuong = isnull(SoLuong, 0) - isnull(TongSoLuong, 0)
	from (select MaSach, sum(SLBan) as TongSoLuong from deleted group by MaSach) as tmp
	where tSach.MaSach = tmp.MaSach
end
...
begin
	...
	Update tHoaDonBan 
	set SoSanPham =  isnull(SoSanPham,0) + isnull(@slbanthem,0) -isnull(@slbanxoa,0)
...
/* Tạo thủ tục có đầu vào là mã sách, năm, đầu ra số lượng sách nhập, số lượng sách bán trong năm đó */
	GO
create or alter procedure Cau9 @masach nvarchar(20),@nam int,@slnhap int output,@slban int output
as begin
	
	select @slnhap = Sum(SLnhap) from tChiTietHDN inner join tHoaDonNhap on tHoaDonNhapThêm trường Tổng tiền tiêu dùng cho bảng khách hàng.SoHDN = tChiTietHDN.SoHDN where Year(NgayNhap) = @nam and tChiTietHDN.MaSach =@masach
	select @slban = Sum(SLBan) from tChiTietHDB inner join tHoaDonBan on tHoaDonBan.SoHDB = tChiTietHDB.SoHDB where Year(NgayBan) = @nam and tChiTietHDB.MaSach =@masach	
end 
...
create or alter procedure cau11_1 @makh nvarchar(30), @nam datetime, @slhd int output, @tongtien money output
as 
begin 
	select @slhd = count(SoHDB) from tHoaDonBan where MaKH = @makh and year(NgayBan) = @nam
	select @tongtien = sum(SLBan * S.DonGiaBan) from tHoaDonBan as HD join tChiTietHDB as CTHD on
